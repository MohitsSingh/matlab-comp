...
elseif method==3    
    [h_matrix_order, cell_idx_max]=sort(h_matrix,3, 'descend');
    for xPixel=1:xPixels
        for yPixel=1:yPixels
        SINR_temp=zeros(1,number_snapshots);
            for NSS = 1:number_snapshots
                h1=h_matrix_order(yPixel,xPixel,1, NSS);
                h2=w_pha*h_matrix_order(yPixel,xPixel,2, NSS);
    %             h2=[h2_1;h2_1;h2_1;h2_1];
            ...
            % computing desired signal
                Rx_level_max=db2pow(BS_tx_power-30)*(hMax^2)/50;   %in power
            % Computing SINR
                SNR_temp(NSS)= pow2db(Rx_level_max) - noise_DL;
